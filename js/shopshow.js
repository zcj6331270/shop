...
$(function () {
	...
	$('.dianji').on('click', 'button', function (e) {
		...
		if (target.prop('tagName') == 'BUTTON') {
			...
			if (JSON.parse(localStorage.getItem('xiaomi3'))) {
				var arr = JSON.parse(localStorage.getItem('xiaomi3'))
				var num = 1;
				var fool;
				var price = parseInt($('.qian').text());
				var iphoneColor = $("#yansetu").text();
				var address = $('.sheng').text() + $('.shi').text() + $('.qu').text() + $('.jiedao').text();
				var name = $('.type').text();
				...
				auto: for (var i = 0; i < arr.length; i++) {
					for (var k in arr[i]) {
						if (arr[i].name == name && arr[i].type == type && arr[i].iphoneColor == iphoneColor && arr[i].address == address && arr[i].price == price) {
							num = arr[i].tok + 1;
							data = {
								name: name,
								type: type,
							...
							localStorage.setItem('xiaomi3', arr);
							fool = i;
							console.log(i)
							break auto;
						}
				...
				console.log(arr);
				localStorage.setItem('xiaomi3', arr);
				console.log(localStorage.xiaomi3)
			}
		}
